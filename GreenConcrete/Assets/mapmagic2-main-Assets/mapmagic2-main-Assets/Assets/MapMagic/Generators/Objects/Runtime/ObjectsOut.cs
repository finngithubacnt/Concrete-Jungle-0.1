...
namespace MapMagic.Nodes.ObjectsGenerators
{
	...
	// should be mostly replaced with PositioningSettings
	{
		...
		//advanced:
		public enum RandomBase { Id, Hash };
		public RandomBase randomBase;
		public bool randomBaseSerialized = false; //to switch to Hash when updating older graphs
		...
		public static void Finalize (TileData data, StopToken stop)
		{
				...
				in data.Outputs<ObjectsOutput,TransitionsList,MatrixWorld>(typeof(ObjectsOutput), inSubs:true))
			{
				...
				for (int t=0; t<trns.count; t++)
				{
					...
					float rnd = random.Random(output.randomBase==RandomBase.Hash ? trn.hash : trn.id);
