	...
	public sealed class  UnitySplinesOutput214 : OutputGenerator, IInlet<SplineSys>  //virtually standard generator (never writes to products)
	{
		...
		public class ApplyData : IApplyData
		{
			...
			public void Apply(Terrain terrain)
			{
				...
				splineContainer.Splines = ConvertSplineSysToSpline(spline, offset:terrain.transform.position);
            ...
            #region Copy of the creation code from Unity Splines Editor (brought here to be called in runtime)
				...
				static Spline[] ConvertSplineSysToSpline (SplineSys sys, Vector3 offset=new Vector3())
				{
					...
					for (int s=0; s<splines.Length; s++)
					{
						...
						if (sysLine.segments.Length == 1)
						{
							...
							knots[0] = new BezierKnot(sysLine.segments[0].start.pos-offset, -sysLine.segments[0].start.dir, sysLine.segments[0].start.dir);
							knots[1] = new BezierKnot(sysLine.segments[0].end.pos-offset, sysLine.segments[0].end.dir, -sysLine.segments[0].end.dir);
						}
						...
						else
						{
							...
							for (int k=1; k<knots.Length-1; k++)
								knots[k] = new BezierKnot(sysLine.segments[k].start.pos-offset, sysLine.segments[k-1].end.dir, sysLine.segments[k].start.dir);

							//end
							Segment lastSegment = sysLine.segments[sysLine.segments.Length-1];
							knots[knots.Length-1] = new BezierKnot(lastSegment.end.pos-offset, lastSegment.end.dir, -lastSegment.end.dir);
						}
